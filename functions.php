<?php
//функции для CICADA-SEARCH
require 'enhanced_functions.php';
...
function extract_social_links($sourcecode){
  /* Это действительно простой код, а пока я буду работать над ним в следующей версии.
     На данный момент поддерживаются только эти социальные сети:
      - Facebook
      - Твиттер
      - Instagram
      - YouTube
  ...
  */
  ...
  foreach ($links as $link) {
    ...
    elseif (strpos ($link, "pinterest.com/") !== false) {
      ...
      // Я знаю, что это не имеет ничего общего с кодом, но опять же я люблю комментарии; __; приятно тратить время: p
        }
  }
  ...
  elseif ($total_social_link_count == "1") {
    // Столько, сколько я ненавижу признать, грамматик важен: p
...
function checkSecurityHeaders($url) {
  ...
  foreach ($security_headers as $header => $description) {
    if (isset($headers[$header])) {
      echo $bold . $lblue . "  [$header]: " . $fgreen . $headers[$header] . $cln . "\n";
    } else {
      echo $bold . $lblue . "  [$header]: " . $red . "NOT FOUND (Missing)" . $cln . "\n";
    }
  }
}

// Сканирование на backup файлы
function scanBackupFiles($reallink) {
  global $bold, $lblue, $fgreen, $red, $cln, $yellow;
  
  echo $bold . $yellow . "\n[*] Scanning for Backup/Config Files:\n" . $cln;
  
  $dangerous_files = array(
    '/backup.sql', '/backup.zip', '/backup.tar.gz',
    '/.git', '/.svn', '/.env',
    '/config.php', '/wp-config.php', '/settings.ini',
    '/.htaccess', '/web.config',
  ...
  foreach ($dangerous_files as $file) {
    ...
    if ($httpCode == 200 || $httpCode == 403) {
      $found_files[] = array('file' => $file, 'code' => $httpCode);
    }
  }
  
  if (count($found_files) > 0) {
    echo $bold . $red . "  [!] Found " . count($found_files) . " suspicious files:\n" . $cln;
    foreach ($found_files as $found) {
      echo $bold . $lblue . "    - " . $fgreen . $found['file'] . $yellow . " (HTTP " . $found['code'] . ")" . $cln . "\n";
    }
  } else {
    echo $bold . $fgreen . "  [+] No suspicious files found\n" . $cln;
  }
}

// Проверка SSL/TLS версии
function checkSSLVersion($domain) {
  global $bold, $lblue, $fgreen, $red, $cln, $yellow;
  
  echo $bold . $yellow . "\n[*] SSL/TLS Analysis:\n" . $cln;
  
  $command = "echo | openssl s_client -connect " . $domain . ":443 -tls1 2>/dev/null | grep 'Protocol'";
  $output = shell_exec($command);
  
  if (strpos($output, 'TLSv1.2') !== false || strpos($output, 'TLSv1.3') !== false) {
    echo $bold . $fgreen . "  [+] Modern TLS Version: " . trim($output) . $cln . "\n";
  } else {
    echo $bold . $red . "  [!] Outdated TLS Version: " . trim($output) . $cln . "\n";
  }
}

// Сканирование на Open Redirect
function checkOpenRedirect($reallink) {
  global $bold, $lblue, $fgreen, $red, $cln, $yellow;
  ...
  foreach ($test_urls as $param) {
    ...
    if (isset($info['redirect_url']) && strpos($info['redirect_url'], 'google.com') !== false) {
      echo $bold . $red . "  [!] Potential Open Redirect: " . $param . $cln . "\n";
      $vulnerable = true;
    }
  }
  
  if (!$vulnerable) {
    echo $bold . $fgreen . "  [+] No obvious Open Redirect found\n" . $cln;
  }
}

// Поиск Information Disclosure
function checkInfoDisclosure($reallink) {
  global $bold, $lblue, $fgreen, $red, $cln, $yellow;
  
  echo $bold . $yellow . "\n[*] Information Disclosure Check:\n" . $cln;
  
  $disclosure_files = array(
  ...
  foreach ($disclosure_files as $file => $desc) {
    $url = $reallink . $file;
    $handle = curl_init($url);
    curl_setopt($handle, CURLOPT_RETURNTRANSFER, TRUE);
    curl_setopt($handle, CURLOPT_TIMEOUT, 5);
    ...
    if ($httpCode == 200) {
      echo $bold . $red . "  [!] Found: " . $file . " (" . $desc . ")" . $cln . "\n";
    }
  }
}
